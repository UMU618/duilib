...
void CPreviewWnd::Notify(TNotifyUI& msg) {
  if (msg.sType == DUI_MSGTYPE_CLICK) {
    MessageBox(m_hWnd, msg.pSender->GetName(), msg.pSender->GetClass(), MB_OK);
  }
}

LRESULT CPreviewWnd::OnDestroy(UINT /*uMsg*/,
                               WPARAM /*wParam*/,
                               LPARAM /*lParam*/,
                               BOOL& bHandled) {
  PostQuitMessage(0);
...
LRESULT CPreviewWnd::OnKeyDown(UINT /*uMsg*/,
                               WPARAM wParam,
                               LPARAM /*lParam*/,
                               BOOL& bHandled) {
  if (wParam == VK_ESCAPE) {
...
LRESULT CPreviewWnd::OnNcHitTest(UINT uMsg,
                                 WPARAM wParam,
                                 LPARAM lParam,
                                 BOOL& bHandled) {
  POINT pt;
