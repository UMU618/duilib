...
std::string UnicodeToAnsi(const std::wstring& strSource) {
  int nLength = ::WideCharToMultiByte(CP_ACP, 0, strSource.data(), -1, NULL, 0,
                                      NULL, FALSE);
  ...
  ::WideCharToMultiByte(CP_ACP, 0, strSource.data(), -1, &vecResult[0], nLength,
                        NULL, FALSE);
...
std::wstring AnsiToUnicode(const std::string& strSource) {
  ...
  ::MultiByteToWideChar(CP_ACP, 0, strSource.data(), strSource.size(),
                        &strResult[0], nLength);
...
std::wstring Utf8ToUnicode(const std::string& strSrouce) {
  int nLength =
      ::MultiByteToWideChar(CP_UTF8, 0, strSrouce.data(), -1, NULL, 0);
  ...
  std::wstring strResult(nLength, 0);
  ::MultiByteToWideChar(CP_UTF8, 0, strSrouce.data(), strSrouce.size(),
                        &strResult[0], nLength);
...
std::string UnicodeToUtf8(const std::wstring& strSource) {
  int nLength = ::WideCharToMultiByte(CP_UTF8, 0, strSource.data(), -1, NULL, 0,
                                      NULL, FALSE);
  ...
  ::WideCharToMultiByte(CP_UTF8, 0, strSource.data(), -1, &strResult[0],
                        nLength, NULL, FALSE);
...
bool UrlEncode(const char* szSrc, char* pBuf, int cbBufLen, bool bUpperCase) {
  ...
  int cbUTF8 = WideCharToMultiByte(CP_UTF8, 0, pUnicode, cchWideChar, NULL, 0,
                                   NULL, NULL);
  ...
  WideCharToMultiByte(CP_UTF8, 0, pUnicode, cchWideChar, pUTF8, cbUTF8 + 1,
                      NULL, NULL);
  pUTF8[cbUTF8] = '\0';
  ...
  while (*pSrc && cbDest < cbBufLen - 1) {
    ...
    if (isalpha(c) || isdigit(c) || c == '-' || c == '.' || c == '~') {
      ...
      *pDest = '+';
      ...
      pDest[2] =
          ((c & 0xF) >= 0xA) ? ((c & 0xF) - 10 + baseChar) : ((c & 0xF) + '0');
      pDest += 3;
      cbDest += 3;
    }
    ++pSrc;
...
bool UrlDecode(const char* szSrc, char* pBuf, int cbBufLen) {
  ...
  *pDest = '\0';
  ...
  int cchWideChar =
      MultiByteToWideChar(CP_UTF8, 0, (LPCSTR)pUTF8, cbDest, NULL, 0);
  ...
  WideCharToMultiByte(CP_ACP, 0, pUnicode, cchWideChar, pBuf, cbBufLen, NULL,
                      NULL);
  free(pUTF8);
  free(pUnicode);
  return true;
...
std::string URLEncodeGB2312Forspace(
    const char* szSrc /*, char* pBuf, int cbBufLen*/) {
  std::string sOut;
  for (size_t ix = 0; ix < strlen(szSrc); ix++) {
    BYTE buf[4];
    memset(buf, 0, 4);
...
char* Utf8ToGBK(const char* strUtf8) {
  ...
  MultiByteToWideChar(CP_UTF8, 0, /*(LPCTSTR)*/ strUtf8, -1, (LPWSTR)wszGBK,
                      len);
  len =
...
char* GBKToUtf8(const char* strGBK) {
  ...
  MultiByteToWideChar(CP_ACP, 0, /*(LPCTSTR)*/ strGBK, -1, (LPWSTR)wszUtf8,
                      len);
  len = WideCharToMultiByte(CP_UTF8, 0, (LPCWSTR)wszUtf8, -1, NULL, 0, NULL,
                            NULL);
  char* szUtf8 = new char[len + 1];
  memset(szUtf8, 0, len + 1);
  WideCharToMultiByte(CP_UTF8, 0, (LPCWSTR)wszUtf8, -1, (LPSTR)szUtf8, len,
                      NULL, NULL);
  return szUtf8;
}
  ...
  try {
    ...
    while (npos != string::npos) {
      ...
      if (strValue.length() == 4) {
        if ((strValue.at(0) >= '0' && strValue.at(0) <= '9') ||
            (strValue.at(0) >= 'a' && strValue.at(0) <= 'f') ||
            (strValue.at(0) >= 'A' && strValue.at(0) <= 'F')) {
          if ((strValue.at(1) >= '0' && strValue.at(1) <= '9') ||
              (strValue.at(1) >= 'a' && strValue.at(1) <= 'f') ||
              (strValue.at(1) >= 'A' && strValue.at(1) <= 'F')) {
            if ((strValue.at(2) >= '0' && strValue.at(2) <= '9') ||
                ((strValue.at(2) >= 'a' && strValue.at(2) <= 'f')) ||
                (strValue.at(2) >= 'A' && strValue.at(2) <= 'F')) {
              if ((strValue.at(3) >= '0' && strValue.at(3) <= '9') ||
                  ((strValue.at(3) >= 'a' && strValue.at(3) <= 'f')) ||
                  (strValue.at(3) >= 'A' && strValue.at(3) <= 'F')) {
                sscanf_s(strValue.c_str(), "%4x", &wch);
                strAnsi.replace(strAnsi.begin() + npos,
                                strAnsi.begin() + npos + 6, _A(&wch));
