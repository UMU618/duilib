...
CListUI::CListUI()
  ...
  m_ListInfo.nFont = -1;
  m_ListInfo.uTextStyle =
      DT_VCENTER | DT_SINGLELINE;  // m_uTextStyle(DT_VCENTER | DT_END_ELLIPSIS)
...
bool CListUI::SetItemIndex(CControlUI* pControl, int iIndex) {
  ...
  if (m_iCurSel >= 0)
    pSelectedListItem = static_cast<IListItemUI*>(
        GetItemAt(m_iCurSel)->GetInterface(DUI_CTR_ILISTITEM));
  ...
  for (int i = iMinIndex; i < iMaxIndex + 1; ++i) {
    ...
    IListItemUI* pListItem =
        static_cast<IListItemUI*>(p->GetInterface(DUI_CTR_ILISTITEM));
    if (pListItem != NULL) {
...
bool CListUI::SetMultiItemIndex(CControlUI* pStartControl,
  ...
  if (pStartControl->GetInterface(DUI_CTR_LISTHEADER) != NULL)
    return CVerticalLayoutUI::SetMultiItemIndex(pStartControl, iCount,
                                                iNewStartIndex);
  // We also need to recognize header sub-items
...
bool CListUI::Add(CControlUI* pControl) {
  ...
  IListItemUI* pListItem =
      static_cast<IListItemUI*>(pControl->GetInterface(DUI_CTR_ILISTITEM));
  if (pListItem != NULL) {
...
bool CListBodyUI::SortItems(PULVCompareFunc pfnCompare,
                            UINT_PTR dwData,
                            int& iCurSel) {
  if (!pfnCompare)
    return false;
  ...
  qsort_s(m_items.GetData(), m_items.GetSize(), sizeof(CControlUI*),
          CListBodyUI::ItemComareFunc, this);
  if (pCurSelControl)
...
void CListBodyUI::SetPos(RECT rc, bool bNeedInvalidate) {
  ...
  if (m_pOwner) {
    ...
    if (pInfo != NULL) {
      ...
      if (pInfo->nColumns > 0) {
        szAvailable.cx = pInfo->rcColumn[pInfo->nColumns - 1].right -
                         pInfo->rcColumn[0].left;
      }
    }
  }
...
bool CListBodyUI::DoPaint(HDC hDC,
                          const RECT& rcPaint,
    ...
    if (!::IntersectRect(&rcTemp, &rcPaint, &rc)) {
      ...
      for (int it = 0; it < m_items.GetSize(); it++) {
        ...
        if (!pControl->IsVisible())
          continue;
        if (!pControl->IsFloat()) {
          IListItemUI* pListItem = static_cast<IListItemUI*>(
              pControl->GetInterface(DUI_CTR_ILISTITEM));
          if (pListItem != NULL) {
          ...
          if (pListInfo && pListInfo->iHLineSize > 0) {
            ...
            RECT rcBottomLine = {
                rcPos.left, rcPos.bottom + rcPadding.bottom, rcPos.right,
            ...
            if (::IntersectRect(&rcTemp, &rcPaint, &rcBottomLine)) {
              ...
              CRenderEngine::DrawLine(hDC, rcBottomLine, pListInfo->iHLineSize,
  ...
  if (m_pHorizontalScrollBar != NULL) {
    ...
    if (m_pHorizontalScrollBar->IsVisible()) {
      if (::IntersectRect(&rcTemp, &rcPaint,
                          &m_pHorizontalScrollBar->GetPos())) {
...
SIZE CListHeaderUI::EstimateSize(SIZE szAvailable) {
  ...
  if (cXY.cy == 0 && m_pManager != NULL) {
    for (int it = 0; it < m_items.GetSize(); it++) {
      cXY.cy = MAX(
          cXY.cy,
          static_cast<CControlUI*>(m_items[it])->EstimateSize(szAvailable).cy);
    }
...
CListHeaderItemUI::CListHeaderItemUI()
    : m_bDragable(true),
      ...
      // UMU
      column_text_style_(DT_VCENTER | DT_CENTER | DT_SINGLELINE),
      m_dwTextColor(0),
      m_dwSepColor(0),
      m_iFont(-1),
      m_bShowHtml(false) {
...
// UMU {
DWORD CListHeaderItemUI::GetColumnTextStyle() const {
  return column_text_style_;
}

void CListHeaderItemUI::SetColumnTextStyle(UINT text_style) {
  column_text_style_ = text_style;
  Invalidate();
}
// UMU }
...
void CListHeaderItemUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue) {
  ...
  } else if (0 == wcscmp(pstrName, L"columnalign")) { // UMU: add
    if (_tcsstr(pstrValue, _T("left")) != NULL) {
      column_text_style_ &= ~(DT_CENTER | DT_RIGHT);
      column_text_style_ |= DT_LEFT;
    }
    if (_tcsstr(pstrValue, _T("center")) != NULL) {
      column_text_style_ &= ~(DT_LEFT | DT_RIGHT);
      column_text_style_ |= DT_CENTER;
    }
    if (_tcsstr(pstrValue, _T("right")) != NULL) {
      column_text_style_ &= ~(DT_LEFT | DT_CENTER);
      column_text_style_ |= DT_RIGHT;
    }
...
SIZE CListHeaderItemUI::EstimateSize(SIZE szAvailable) {
  if (m_cxyFixed.cy == 0)
    return CDuiSize(m_cxyFixed.cx,
                    m_pManager->GetDefaultFontInfo()->tm.tmHeight + 8);
  return CControlUI::EstimateSize(szAvailable);
}

RECT CListHeaderItemUI::GetThumbRect() const {
  if (m_iSepWidth >= 0)
    return CDuiRect(m_rcItem.right - m_iSepWidth, m_rcItem.top, m_rcItem.right,
                    m_rcItem.bottom);
  else
    return CDuiRect(m_rcItem.left, m_rcItem.top, m_rcItem.left - m_iSepWidth,
                    m_rcItem.bottom);
}
...
void CListHeaderItemUI::PaintStatusImage(HDC hDC) {
  ...
  if (m_iSepWidth > 0) {
    ...
    if (!DrawImage(hDC, m_diSep)) {
      if (m_dwSepColor != 0) {
        RECT rcSepLine = {rcThumb.left + m_iSepWidth / 2, rcThumb.top,
                          rcThumb.left + m_iSepWidth / 2, rcThumb.bottom};
        CRenderEngine::DrawLine(hDC, rcSepLine, m_iSepWidth,
                                GetAdjustColor(m_dwSepColor));
      }
    }
  }
}
...
void CListElementUI::SetOwner(CControlUI* pOwner) {
  if (pOwner != NULL)
    m_pOwner =
        static_cast<IListOwnerUI*>(pOwner->GetInterface(DUI_CTR_ILISTOWNER));
}
...
void CListElementUI::DoEvent(TEventUI& event) {
  ...
  // parent but to the "attached" list. A list may actually embed several
  // components in its path to the item, but key-presses etc. needs to go to the
  // actual list.
  if (m_pOwner != NULL)
    m_pOwner->DoEvent(event);
  else
...
SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable) {
  ...
  if (m_uFixedHeightLast != pInfo->uFixedHeight ||
      m_nFontLast != pInfo->nFont || m_uTextStyleLast != pInfo->uTextStyle ||
  ...
  if (m_bNeedEstimateSize) {
    ...
    if ((pInfo->uTextStyle & DT_SINGLELINE) != 0) {
      if (m_cxyFixedLast.cy == 0) {
        m_cxyFixedLast.cy =
            m_pManager->GetFontInfo(pInfo->nFont)->tm.tmHeight + 8;
        m_cxyFixedLast.cy +=
            pInfo->rcTextPadding.top + pInfo->rcTextPadding.bottom;
      }
      if (m_cxyFixedLast.cx == 0) {
        ...
        if (pInfo->bShowHtml) {
          ...
          CRenderEngine::DrawHtmlText(
              m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, 0, NULL,
              NULL, nLinks, pInfo->nFont,
              DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
        } else {
          CRenderEngine::DrawText(
              m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, 0,
              pInfo->nFont,
              DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
        }
...
CListTextElementUI::CListTextElementUI()
    : m_nLinks(0), m_nHoverLink(-1), m_pOwner(NULL) {
...
SIZE CListTextElementUI::EstimateSize(SIZE szAvailable) {
  ...
  if (cxyFixed.cx == 0 && pInfo->nColumns > 0) {
    cxyFixed.cx =
        pInfo->rcColumn[pInfo->nColumns - 1].right - pInfo->rcColumn[0].left;
    ...
    if ((pInfo->uTextStyle & DT_SINGLELINE) != 0) {
      ...
      if (m_cxyFixedLast.cx == 0) {
        ...
        if (pInfo->bShowHtml) {
          int nLinks = 0;
          CRenderEngine::DrawHtmlText(
              m_pManager->GetPaintDC(), m_pManager, rcText, strText, 0, NULL,
              NULL, nLinks, pInfo->nFont,
              DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
        } else {
          CRenderEngine::DrawText(
              m_pManager->GetPaintDC(), m_pManager, rcText, strText, 0,
              pInfo->nFont,
              DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
        }
        m_cxyFixedLast.cx = rcText.right - rcText.left +
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem) {
  ...
  // UMU
  CListHeaderUI* header = m_pOwner->GetHeader();

  m_nLinks = 0;
  int nLinks = lengthof(m_rcLinks);
  if (pInfo->nColumns > 0) {
    for (int i = 0; i < pInfo->nColumns; i++) {
      RECT rcItem = {pInfo->rcColumn[i].left, m_rcItem.top,
                     pInfo->rcColumn[i].right, m_rcItem.bottom};
      if (pInfo->iVLineSize > 0 && i < pInfo->nColumns - 1) {
        RECT rcLine = {rcItem.right - pInfo->iVLineSize / 2, rcItem.top,
                       rcItem.right - pInfo->iVLineSize / 2, rcItem.bottom};
        CRenderEngine::DrawLine(hDC, rcLine, pInfo->iVLineSize,
                                GetAdjustColor(pInfo->dwVLineColor));
      ...
      // UMU {
      CControlUI* header_item = header->GetItemAt(i);
      UINT text_style = pInfo->uTextStyle;
      if (nullptr != header_item->GetInterface(DUI_CTR_LISTHEADERITEM)) {
        CListHeaderItemUI* header_item_ui =
            static_cast<CListHeaderItemUI*>(header_item);
        text_style = header_item_ui->GetColumnTextStyle();
      }
      // UMU }

      if (pInfo->bShowHtml)
        CRenderEngine::DrawHtmlText(hDC, m_pManager, rcItem, strText.GetData(),
                                    iTextColor, &m_rcLinks[m_nLinks],
                                    &m_sLinks[m_nLinks], nLinks, pInfo->nFont,
                                    // UMU: pInfo->uTextStyle);
                                    text_style);
      else
        CRenderEngine::DrawText(hDC, m_pManager, rcItem, strText.GetData(),
                                iTextColor, pInfo->nFont, // UMU: pInfo->uTextStyle);
                                text_style);
    ...
    if (pInfo->bShowHtml)
      ...
      CRenderEngine::DrawText(hDC, m_pManager, rcItem, strText.GetData(),
                              iTextColor, pInfo->nFont, pInfo->uTextStyle);
...
void CListContainerElementUI::SetAttribute(LPCTSTR pstrName,
                                           LPCTSTR pstrValue) {
  if (_tcscmp(pstrName, _T("selected")) == 0)
    Select();
...
void CListHBoxElementUI::SetPos(RECT rc, bool bNeedInvalidate) {
  ...
  if (pInfo->nColumns > 0) {
    ...
    for (int it2 = 0; it2 < m_items.GetSize(); it2++) {
      ...
      RECT rcPadding = pControl->GetPadding();
      RECT rcItem = {pInfo->rcColumn[iColumnIndex].left + rcPadding.left,
    ...
    for (int it2 = 0; it2 < m_items.GetSize(); it2++) {
      ...
      RECT rcPadding = pControl->GetPadding();
      RECT rcItem = {
          m_rcItem.left + rcPadding.left, m_rcItem.top + rcPadding.top,
