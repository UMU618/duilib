...
Color ARGB2Color(DWORD dwColor) {
  return Color(HIBYTE((dwColor) >> 16), GetBValue(dwColor), GetGValue(dwColor),
               GetRValue(dwColor));
}
...
SIZE CLabelUI::EstimateSize(SIZE szAvailable) {
  ...
  if (m_bNeedEstimateSize) {
    ...
    if ((m_uTextStyle & DT_SINGLELINE) != 0) {
      ...
      if (m_cxyFixedLast.cx == 0) {
        ...
        if (m_bShowHtml) {
          ...
          CRenderEngine::DrawText(
              m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, 0, m_iFont,
              DT_CALCRECT | m_uTextStyle & ~DT_RIGHT & ~DT_CENTER);
        }
        m_cxyFixedLast.cx = rcText.right - rcText.left + m_rcTextPadding.left +
      ...
      m_cxyFixedLast.cy = rcText.bottom - rcText.top + m_rcTextPadding.top +
...
void CLabelUI::PaintText(HDC hDC) {
  ...
  if (!GetEnabledEffect()) {
    ...
    if (IsEnabled()) {
      ...
      if (m_bShowHtml)
        CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, m_sText,
                                    m_dwDisabledTextColor, NULL, NULL, nLinks,
                                    m_iFont, m_uTextStyle);
      else
        CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText,
    ...
    RectF nRc((float)rc.left, (float)rc.top, (float)rc.right - rc.left,
    ...
    LinearGradientBrush nLineGrBrushA(
        Point(GetGradientAngle(), 0), Point(0, nGradientLength),
        ARGB2Color(GetTextShadowColorA()),
        ARGB2Color(GetTextShadowColorB() == -1 ? GetTextShadowColorA()
                                               : GetTextShadowColorB()));
    ...
    if (GetEnabledLuminous()) {
      ...
      g1.Clear(Color(0));
      g1.DrawImage(&Bit2, (int)m_ShadowOffset.X, (int)m_ShadowOffset.Y,
                   (int)nRc.Width, (int)nRc.Height);
      g1.SetTextRenderingHint(TextRenderingHintAntiAlias);
    ...
    if (GetEnabledStroke() && GetStrokeColor() > 0) {
      LinearGradientBrush nLineGrBrushStroke(
          Point(GetGradientAngle(), 0), Point(0, rc.bottom - rc.top + 2),
          ARGB2Color(GetStrokeColor()), ARGB2Color(GetStrokeColor()));
#ifdef _UNICODE
      nRc.Offset(-1, 0);
      nGraphics.DrawString(m_sText, m_sText.GetLength(), &nFont, nRc, &format,
                           &nLineGrBrushStroke);
      nRc.Offset(2, 0);
      nGraphics.DrawString(m_sText, m_sText.GetLength(), &nFont, nRc, &format,
                           &nLineGrBrushStroke);
      ...
      int iLen = wcslen(m_pWideText);
      nRc.Offset(-1, 0);
      nGraphics.DrawString(m_pWideText, iLen, &nFont, nRc, &format,
                           &nLineGrBrushStroke);
      nRc.Offset(2, 0);
      nGraphics.DrawString(m_pWideText, iLen, &nFont, nRc, &format,
                           &nLineGrBrushStroke);
    ...
    if (GetEnabledShadow() &&
      ...
      nGraphics.DrawString(m_sText, m_sText.GetLength(), &nFont, nShadowRc,
                           &format, &nLineGrBrushA);
    ...
    if (GetEnabledShadow() &&
        (GetTextShadowColorA() > 0 || GetTextShadowColorB() > 0))
      nGraphics.DrawString(m_pWideText, iLen, &nFont, nShadowRc, &format,
                           &nLineGrBrushA);
...
void CLabelUI::SetEnabledEffect(bool _EnabledEffect) {
  ...
  if (m_EnableEffect) {
    ...
    m_pWideText = new WCHAR[iLen + 1];
    ::ZeroMemory(m_pWideText, (iLen + 1) * sizeof(WCHAR));
    ::MultiByteToWideChar(CP_ACP, 0, m_sText.GetData(), -1, (LPWSTR)m_pWideText,
                          iLen);
#endif
  }
  Invalidate();
