...
CButtonUI::CButtonUI()
    : m_uButtonState(0),
      ...
      pushed_bk_color_(0),
      hot_border_color_(0),
      pushed_border_color_(0),
      disabled_border_color_(0),
      hot_font_(-1),
      pushed_font_(-1)
...
void CButtonUI::DoEvent(TEventUI& event) {
  ...
  if (event.Type == UIEVENT_CONTEXTMENU) {
    if (IsContextMenuUsed() && IsEnabled()) {
      m_pManager->SendNotify(this, DUI_MSGTYPE_MENU, event.wParam,
                             event.lParam);
    }
    return;
  }
...
SIZE CButtonUI::EstimateSize(SIZE szAvailable) {
  if (m_cxyFixed.cy == 0)
    return CDuiSize(m_cxyFixed.cx,
                    m_pManager->GetFontInfo(GetFont())->tm.tmHeight + 8);
...
void CButtonUI::PaintText(HDC hDC) {
  ...
  if ((m_uButtonState & UISTATE_PUSHED) != 0) {
    ...
    if (GetFocusedTextColor() != 0) {
      clrColor = GetFocusedTextColor();
    }
  }

  // if( ((m_uButtonState & UISTATE_PUSHED) != 0) && (GetPushedTextColor() != 0)
  // ) 	clrColor = GetPushedTextColor();  else if( ((m_uButtonState &
  // UISTATE_HOT) != 0) && (GetHotTextColor() != 0) ) 	clrColor =
  //GetHotTextColor();  else if( ((m_uButtonState & UISTATE_FOCUSED) != 0) &&
  // (GetFocusedTextColor() != 0) ) 	clrColor = GetFocusedTextColor();

  if (m_bShowHtml)
    CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, m_sText, clrColor, NULL,
                                NULL, nLinks, font, m_uTextStyle);
  else
    CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText, clrColor, font,
                            m_uTextStyle);
}
...
void CButtonUI::PaintBkColor(HDC hDC) {
  if (!IsEnabled() || (m_uButtonState & UISTATE_DISABLED) != 0) {
    COLORREF color = GetDisabledBkColor();
    if (0 == color) {
      color = m_dwBackColor;
    }
    CRenderEngine::DrawColor(hDC, m_rcItem, GetAdjustColor(color));
  } else if ((m_uButtonState & UISTATE_PUSHED) != 0) {
    if (pushed_bk_color_ != 0) {
      CRenderEngine::DrawColor(hDC, m_rcPaint,
                               GetAdjustColor(pushed_bk_color_));
    }
  } else if ((m_uButtonState & UISTATE_HOT) != 0) {
    if (m_dwHotBkColor != 0) {
...
void CButtonUI::PaintStatusImage(HDC hDC) {
  ...
  // UMU
  if (!m_diBk.sDrawString.IsEmpty()) {
    if (!DrawImage(hDC, m_diBk)) {
      m_diBk.sDrawString.Empty();
    }
  }
  ...
  if ((m_uButtonState & UISTATE_DISABLED) != 0) {
    ...
    //	CRenderEngine::DrawColor(hDC, m_rcPaint,
    //GetAdjustColor(m_dwHotBkColor)); 	return;
    //}
...
// UMU {
void CButtonUI::SetPushedBkColor(COLORREF color) {
  pushed_bk_color_ = color;
}

COLORREF CButtonUI::GetPushedBkColor() const {
  return pushed_bk_color_;
}

void CButtonUI::SetHotBorderColor(COLORREF color) {
  hot_border_color_ = color;
}

COLORREF CButtonUI::GetHotBorderColor() const {
  return hot_border_color_;
}

void CButtonUI::SetPushedBorderColor(COLORREF color) {
  pushed_border_color_ = color;
}

COLORREF CButtonUI::GetPushedBorderColor() const {
  return pushed_border_color_;
}

void CButtonUI::SetDisabledBorderColor(COLORREF color) {
  disabled_border_color_ = color;
}

COLORREF CButtonUI::GetDisabledBorderColor() const {
  return disabled_border_color_;
}

void CButtonUI::SetHotFont(int font) {
  hot_font_ = font;
}
...
void CButtonUI::SetPushedFont(int font) {
  pushed_font_ = font;
}
...
void CButtonUI::PaintBorder(HDC hDC) {
  if (0 != (m_uButtonState & UISTATE_DISABLED) || !IsEnabled()) {
    CControlUI::PaintBorder(hDC, disabled_border_color_, m_rcBorderSize,
                            m_nBorderStyle);
  } else {
    if (0 != (m_uButtonState & UISTATE_PUSHED)) {
      CControlUI::PaintBorder(hDC, pushed_border_color_, m_rcBorderSize,
                              m_nBorderStyle);
    } else if (0 != (m_uButtonState & UISTATE_HOT)) {
      CControlUI::PaintBorder(hDC, hot_border_color_, m_rcBorderSize,
                              m_nBorderStyle);
    } else {
      CControlUI::PaintBorder(hDC, m_dwBorderColor, m_rcBorderSize,
                              m_nBorderStyle);
    }

    if (0 != (m_uButtonState & UISTATE_FOCUSED) || IsFocused()) {
      CControlUI::PaintBorder(hDC, m_dwFocusBorderColor, focus_border_sizes_,
                              focus_border_style_);
    }
  }
}
