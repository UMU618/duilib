...
LPVOID CTileLayoutUI::GetInterface(LPCTSTR pstrName) {
  if (_tcscmp(pstrName, DUI_CTR_TILELAYOUT) == 0)
...
void CTileLayoutUI::SetPos(RECT rc, bool bNeedInvalidate) {
  ...
  if (m_nColumnsFixed == 0) {
    if (rc.right - rc.left >= m_szItem.cx) {
      ...
      if (m_nColumns > 1) {
        if (iChildPadding <= 0) {
          iChildPadding =
              (cxNeeded - m_nColumns * m_szItem.cx) / (m_nColumns - 1);
        }
        if (iChildPadding < 0)
          iChildPadding = 0;
      } else {
    ...
    if (m_nColumns > 1) {
      if (iChildPadding <= 0) {
        if (m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible() &&
          ...
          iChildPadding = (rc.right - rc.left - m_nColumns * m_szItem.cx) /
                          (m_nColumns - 1);
        } else {
          iChildPadding =
              (szAvailable.cx - m_nColumns * m_szItem.cx) / (m_nColumns - 1);
        }
      }
      if (iChildPadding < 0)
        iChildPadding = 0;
    ...
    if (nEstimateNum >= m_nColumns)
      ...
      cxNeeded =
          nEstimateNum * m_szItem.cx + (nEstimateNum - 1) * iChildPadding;
    m_nRows = (nEstimateNum - 1) / m_nColumns + 1;
    cyNeeded = m_nRows * m_szItem.cy + (m_nRows - 1) * m_iChildVPadding;
  }
  ...
  //		m_nColumns = (rc.right - rc.left) / m_szItem.cx;
  //		if( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible()
  //) 			m_nColumns = (rc.right - rc.left +
  //m_pHorizontalScrollBar->GetScrollRange() ) / m_szItem.cx;
  //	}
  ...
  //static_cast<CControlUI*>(m_items[it2]); 				if(
  //!pLineControl->IsVisible() ) continue; 				if(
  //pLineControl->IsFloat() ) continue;

  //				RECT rcPadding = pLineControl->GetPadding();
  //				SIZE szAvailable = { rcTile.right - rcTile.left -
  //rcPadding.left - rcPadding.right, 9999 };
  ...
  //				SIZE szTile =
  //pLineControl->EstimateSize(szAvailable);
  //				if( szTile.cx == 0 ) szTile.cx = m_szItem.cx;
  //				if( szTile.cy == 0 ) szTile.cy = m_szItem.cy;
  //				if( szTile.cx < pControl->GetMinWidth() ) szTile.cx =
  //pControl->GetMinWidth();
  //				if( szTile.cx > pControl->GetMaxWidth() ) szTile.cx =
  //pControl->GetMaxWidth(); 				if( szTile.cy <
  //pControl->GetMinHeight() ) szTile.cy = pControl->GetMinHeight();
  //				if( szTile.cy > pControl->GetMaxHeight() ) szTile.cy =
  //pControl->GetMaxHeight();

  //				cyHeight = MAX(cyHeight, szTile.cy + rcPadding.top +
  //rcPadding.bottom);
  ...
  //		SIZE szAvailable = { rcTile.right - rcTile.left, rcTile.bottom -
  //rcTile.top }; 		SIZE szTile =
  //pControl->EstimateSize(szAvailable); 		if( szTile.cx == 0 )
  //szTile.cx = m_szItem.cx; 		if( szTile.cy == 0 ) szTile.cy =
  //m_szItem.cy; 		if( szTile.cx == 0 ) szTile.cx = szAvailable.cx;
  //		if( szTile.cy == 0 ) szTile.cy = szAvailable.cy;
  //		if( szTile.cx < pControl->GetMinWidth() ) szTile.cx =
  //pControl->GetMinWidth();
  //		if( szTile.cx > pControl->GetMaxWidth() ) szTile.cx =
  //pControl->GetMaxWidth();
  //		if( szTile.cy < pControl->GetMinHeight() ) szTile.cy =
  //pControl->GetMinHeight();
  //		if( szTile.cy > pControl->GetMaxHeight() ) szTile.cy =
  //pControl->GetMaxHeight();
