...
bool CWndShadow::Initialize(HINSTANCE hInstance) {
  ...
  s_UpdateLayeredWindow =
      (pfnUpdateLayeredWindow)GetProcAddress(hUser32, "UpdateLayeredWindow");
...
void CWndShadow::Create(HWND hParentWnd) {
  ...
  _ASSERT(FindShadowWindow(hParentWnd) ==
          NULL);  // Only one shadow for each window
  ...
  m_hWnd = CreateWindowEx(
      WS_EX_LAYERED | WS_EX_TRANSPARENT, strWndClassName, NULL,
      /*WS_VISIBLE | */ /*WS_CAPTION | */ WS_POPUPWINDOW, CW_USEDEFAULT, 0, 0,
      0, hParentWnd, NULL, s_hInstance, NULL);

  // Determine the initial show state of shadow according to parent window's
  // state
...
LRESULT CALLBACK CWndShadow::WndProc(HWND hWnd,
                                     UINT uMsg,
                                     WPARAM wParam,
                                     LPARAM lParam) {
...
LRESULT CALLBACK CWndShadow::ParentProc(HWND hwnd,
                                        UINT uMsg,
                                        WPARAM wParam,
                                        LPARAM lParam) {
  ...
  switch (uMsg) {
      ...
      if (pThis->m_Status & SS_VISABLE) {
        ...
        if (pThis->m_pImageInfo) {
          SetWindowPos(pThis->m_hWnd, NULL,
                       ...
                       WndRect.top - pThis->m_rcCorner.top, 0, 0,
                       SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOZORDER);

        } else {
          SetWindowPos(pThis->m_hWnd, NULL,
                       WndRect.left + pThis->m_nxOffset - pThis->m_nSize,
                       WndRect.top + pThis->m_nyOffset - pThis->m_nSize, 0, 0,
                       SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOZORDER);
        }
      }
      break;
    ...
    case WM_SIZE:
      if (pThis->m_Status & SS_ENABLED) {
        ...
        } else if (pThis->m_Status &
                   SS_PARENTVISIBLE)  // Parent maybe resized even if invisible
        {
...
#pragma warning( \
    disable : 4312)  // temporrarily disable the type_cast warning in Win32
  // Call the default(original) window procedure for other messages or messages
...
void CWndShadow::Update(HWND hParent) {
  ...
  if (m_pImageInfo) {
    nShadWndWid =
        WndRect.right - WndRect.left + m_rcCorner.left + m_rcCorner.right;
    nShadWndHei =
        WndRect.bottom - WndRect.top + m_rcCorner.top + m_rcCorner.bottom;
  }
  ...
  if (m_pImageInfo) {
    ...
    RECT rcCorner = {m_rcCorner.left + m_rcHoleOffset.left,
                     m_rcCorner.top + m_rcHoleOffset.top,
                     m_rcCorner.right + m_rcHoleOffset.right,
                     m_rcCorner.bottom + m_rcHoleOffset.bottom};
    CRenderEngine::DrawImage(hMemDC, m_pImageInfo->hBitmap, rc, rc, rcBmpPart,
                             rcCorner, true, 255, true);
  } else {
    MakeShadow((UINT32*)pvBits, hParent, &WndRect);
    hOriBmp = (HBITMAP)SelectObject(hMemDC, hbitmap);
  }

  POINT ptDst = {WndRect.left + m_nxOffset - m_nSize,
                 WndRect.top + m_nyOffset - m_nSize};
  if (m_pImageInfo) {
...
void CWndShadow::MakeShadow(UINT32* pShadBits, HWND hParent, RECT* rcParent) {
  ...
  // the blurred border The algorithm is optimized by assuming parent window is
  // just "one piece" and without "wholes" on it

  // Get the region of parent window,
  HRGN hParentRgn = CreateRectRgn(0, 0, rcParent->right - rcParent->left,
  ...
  int(*ptAnchorsOri)[2] =
      new int[szParent.cy][2];  // anchor points, will not modify during erosion
  ...
  for (int i = 0; i < m_nSharpness - m_nSize; i++) {
    ...
    for (int j = 1; j < nAnchors + 1; j++) {
      ptAnchorsTmp[j][0] =
          max(ptAnchors[j - 1][0], max(ptAnchors[j][0], ptAnchors[j + 1][0])) +
          1;
      ptAnchorsTmp[j][1] =
          min(ptAnchors[j - 1][1], min(ptAnchors[j][1], ptAnchors[j + 1][1])) -
          1;
    }
    ...
    for (int j = 0; j <= 2 * nKernelSize; j++) {
      ...
      if (dLength < nCenterSize)
        *pKernelIter = m_nDarkness << 24 | PreMultiply(m_Color, m_nDarkness);
        ...
        UINT32 nFactor = ((UINT32)(
            (1 - (dLength - nCenterSize) / (m_nSharpness + 1)) * m_nDarkness));
    ...
    if (ptAnchors[i][0] < ptAnchors[i][1]) {
      ...
      for (j = ptAnchors[i][0];
        ...
        for (int k = 0; k <= 2 * nKernelSize; k++) {
          UINT32* pPixel =
              pShadBits +
              (szShadow.cy - i - 1 + nKernelSize - k) * szShadow.cx + j -
              nKernelSize;
  ...
  for (int i = min(nKernelSize, max(m_nSize - m_nyOffset, 0));
       i <
       max(szShadow.cy - nKernelSize,
           min(szParent.cy + m_nSize - m_nyOffset, szParent.cy + 2 * m_nSize));
       i++) {
    UINT32* pLine = pShadBits + (szShadow.cy - i - 1) * szShadow.cx;
    if (i - m_nSize + m_nyOffset < 0 ||
        i - m_nSize + m_nyOffset >=
            szParent.cy)  // Line is not covered by parent window
    {
      ...
      for (int j = ptAnchors[i][0];
           j <
           min(ptAnchorsOri[i - m_nSize + m_nyOffset][0] + m_nSize - m_nxOffset,
               ptAnchors[i][1]);
           j++)
        *(pLine + j) = clCenter;
      for (int j = max(
               ptAnchorsOri[i - m_nSize + m_nyOffset][0] + m_nSize - m_nxOffset,
               0);
           j <
           min(ptAnchorsOri[i - m_nSize + m_nyOffset][1] + m_nSize - m_nxOffset,
               szShadow.cx);
           j++)
        *(pLine + j) = 0;
      for (int j = max(
               ptAnchorsOri[i - m_nSize + m_nyOffset][1] + m_nSize - m_nxOffset,
               ptAnchors[i][0]);
           j < ptAnchors[i][1]; j++)
        *(pLine + j) = clCenter;
    }
...
bool CWndShadow::SetPosition(int NewXOffset, int NewYOffset) {
  if (NewXOffset > 20 || NewXOffset < -20 || NewYOffset > 20 ||
      NewYOffset < -20)
    return false;
