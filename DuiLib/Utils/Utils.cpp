...
CDuiPtrArray::CDuiPtrArray(int iPreallocSize)
    : m_ppVoid(NULL), m_nCount(0), m_nAllocated(iPreallocSize) {
  ASSERT(iPreallocSize >= 0);
  if (iPreallocSize > 0)
    m_ppVoid = static_cast<LPVOID*>(malloc(iPreallocSize * sizeof(LPVOID)));
}

CDuiPtrArray::CDuiPtrArray(const CDuiPtrArray& src)
    : m_ppVoid(NULL), m_nCount(0), m_nAllocated(0) {
...
bool CDuiPtrArray::Add(LPVOID pData) {
  if (++m_nCount >= m_nAllocated) {
    ...
    if (nAllocated == 0)
      nAllocated = 11;
    LPVOID* ppVoid =
        static_cast<LPVOID*>(realloc(m_ppVoid, nAllocated * sizeof(LPVOID)));
    if (ppVoid != NULL) {
...
bool CDuiPtrArray::InsertAt(int iIndex, LPVOID pData) {
  ...
  memmove(&m_ppVoid[iIndex + 1], &m_ppVoid[iIndex],
          (m_nCount - iIndex - 1) * sizeof(LPVOID));
...
bool CDuiPtrArray::Remove(int iIndex, int iCount) {
  ...
  if (iIndex + iCount < m_nCount)
    ::CopyMemory(m_ppVoid + iIndex, m_ppVoid + iIndex + iCount,
                 (m_nCount - iIndex - iCount) * sizeof(LPVOID));
  m_nCount -= iCount;
  return true;
}
...
bool CDuiValArray::Add(LPCVOID pData) {
  if (++m_nCount >= m_nAllocated) {
    ...
    if (nAllocated == 0)
      nAllocated = 11;
    LPBYTE pVoid =
        static_cast<LPBYTE>(realloc(m_pVoid, nAllocated * m_iElementSize));
    if (pVoid != NULL) {
      m_nAllocated = nAllocated;
      m_pVoid = pVoid;
    } else {
  ...
  ::CopyMemory(m_pVoid + ((m_nCount - 1) * m_iElementSize), pData,
               m_iElementSize);
  return true;
}
...
bool CDuiValArray::Remove(int iIndex, int iCount) {
  ...
  if (iIndex + iCount < m_nCount)
    ::CopyMemory(m_pVoid + (iIndex * m_iElementSize),
                 m_pVoid + (iIndex + iCount) * m_iElementSize,
...
void CDuiString::Append(LPCTSTR pstr) {
  ...
  if (nNewLength >= MAX_LOCAL_STRING_LEN) {
    if (m_pstr == m_szBuffer) {
      ...
      m_pstr = static_cast<LPTSTR>(
          realloc(m_pstr, (nNewLength + 1) * sizeof(TCHAR)));
      _tcscat(m_pstr, pstr);
    }
  } else {
    if (m_pstr != m_szBuffer) {
...
const CDuiString& CDuiString::operator=(LPCWSTR lpwStr) {
  if (lpwStr) {
    ...
    if (pstr != NULL)
      ::WideCharToMultiByte(::GetACP(), 0, lpwStr, -1, pstr, cchStr, NULL,
                            NULL);
    Assign(pstr);
  } else {
    Empty();
