  ...
  if (pManager) {
    ...
    for (CMarkupNode node = root.GetChild(); node.IsValid();
      ...
      if (_tcsicmp(pstrClass, _T("Image")) == 0) {
        ...
        if (id >= 0 && pFontName) {
          pManager->AddFont(id, pFontName, size, bold, underline, italic,
                            shared);
          if (defaultfont)
...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot,
  ...
  for (CMarkupNode node = pRoot->GetChild(); node.IsValid();
      ...
      for (int i = 0; i < count; i++) {
        if (!builder.GetMarkup()->IsValid()) {
          if (m_pstrtype != NULL) {  // 使用资源dll，从资源中读取
            ...
            pControl = builder.Create((UINT)id, m_pstrtype, m_pCallback,
                                      pManager, pParent);
          } else
            pControl = builder.Create((LPCTSTR)szValue, (UINT)0, m_pCallback,
                                      pManager, pParent);
        } else
    ...
    //树控件XML解析
      ...
      CTreeNodeUI* pParentNode =
          static_cast<CTreeNodeUI*>(pParent->GetInterface(_T("TreeNode")));
      ...
      if (node.HasAttributes()) {
        ...
        for (int i = 0; i < nAttributes; i++) {
          pNode->SetAttribute(node.GetAttributeName(i),
      ...
      if (node.HasChildren()) {
        ...
        if (pSubControl &&
            _tcsicmp(pSubControl->GetClass(), _T("TreeNodeUI")) != 0) {
          // 					pSubControl->SetFixedWidth(30);
          // 					CHorizontalLayoutUI* pHorz =
          // pNode->GetTreeNodeHoriznotal();
      ...
      if (!pParentNode) {
        CTreeViewUI* pTreeView =
            static_cast<CTreeViewUI*>(pParent->GetInterface(_T("TreeView")));
        ASSERT(pTreeView);
        if (pTreeView == NULL)
          return NULL;
      ...
      switch (cchLen) {
        case 4:
          ...
          // UMU
          else if (_tcscmp(pstrClass, DUI_CTR_ICON) == 0)
            pControl = new CIconUI;
          break;
        case 5:
          ...
          // else if( _tcsicmp(pstrClass, DUI_CTR_FLASH) == 0 )
    ...
    if (pParent != NULL) {
      ...
      if (node.GetAttributeValue(_T("cover"), szValue, cchLen) &&
          _tcscmp(lpValue, _T("true")) == 0) {
        pParent->SetCover(pControl);
      } else {
        CTreeNodeUI* pContainerNode =
            static_cast<CTreeNodeUI*>(pParent->GetInterface(DUI_CTR_TREENODE));
        if (pContainerNode)
          pContainerNode->GetTreeNodeHoriznotal()->Add(pControl);
        else {
          if (pContainer == NULL)
            pContainer = static_cast<IContainerUI*>(
                pParent->GetInterface(DUI_CTR_ICONTAINER));
          ASSERT(pContainer);
          if (pContainer == NULL)
            return NULL;
