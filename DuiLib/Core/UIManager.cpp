...
void tagTDrawInfo::Clear() {
  ...
  sImageName.Empty();
  ::ZeroMemory(&bLoaded,
               sizeof(tagTDrawInfo) - offsetof(tagTDrawInfo, bLoaded));
  uFade = 255;
}
...
CPaintManagerUI::CPaintManagerUI()
  ...
  m_ResInfo.m_dwDefaultSelectedBkColor =
  ...
  if (m_bUseHSL || m_bUseHSL != bUseHSL) {
    ...
    for (int i = 0; i < m_aPreMessages.GetSize(); i++) {
      CPaintManagerUI* pManager =
          static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
      if (pManager != NULL)
...
void CPaintManagerUI::SetTooltipWindowWidth(int iWidth) {
  if (m_iTooltipWidth != iWidth) {
    ...
    if (m_hwndTooltip != NULL && m_iTooltipWidth >= 0) {
      m_iTooltipWidth = (int)::SendMessage(m_hwndTooltip, TTM_SETMAXTIPWIDTH, 0,
                                           m_iTooltipWidth);
    }
  }
}
...
SIZE CPaintManagerUI::GetClientSize() const {
  ...
  return CDuiSize(rcClient.right - rcClient.left,
...
void CPaintManagerUI::SetOpacity(BYTE nOpacity) {
  m_nOpacity = nOpacity;
  if (m_hWndPaint != NULL) {
    typedef BOOL(__stdcall * PFUNCSETLAYEREDWINDOWATTR)(HWND, COLORREF, BYTE,
                                                        DWORD);
...
bool CPaintManagerUI::PreMessageHandler(UINT uMsg,
                                        WPARAM wParam,
                                        LPARAM lParam,
                                        LRESULT& /*lRes*/) {
...
bool CPaintManagerUI::MessageHandler(UINT uMsg,
                                     WPARAM wParam,
                                     LPARAM lParam,
                                     LRESULT& lRes) {
  //#ifdef _DEBUG
  ...
  //       break;
  //    default:
  //       DUITRACE(_T("MSG: %-20s (%08ld)"), DUITRACEMSG(uMsg),
  //       ::GetTickCount());
  //    }
  //#endif
  // Not ready yet?
  ...
  switch (uMsg) {
    ...
    case WM_PAINT: {
      ...
      if (m_bOffscreenPaint && m_hbmpOffscreen == NULL) {
        ...
        if (m_bLayered)
          m_hbmpOffscreen = CRenderEngine::CreateARGB32Bitmap(
              m_hDcPaint, rcClient.right - rcClient.left,
              rcClient.bottom - rcClient.top, &m_pOffscreenBits);
        else
          m_hbmpOffscreen = ::CreateCompatibleBitmap(
              m_hDcPaint, rcClient.right - rcClient.left,
              rcClient.bottom - rcClient.top);
      ...
      if (m_bOffscreenPaint) {
        HBITMAP hOldBitmap =
            (HBITMAP)::SelectObject(m_hDcOffscreen, m_hbmpOffscreen);
        ...
        if (m_bLayered && m_diLayered.pImageInfo == NULL) {
          ...
          for (LONG y = rcClient.bottom - rcPaint.bottom;
            ...
            for (LONG x = rcPaint.left; x < rcPaint.right; ++x) {
              pOffscreenBits =
                  m_pOffscreenBits + y * (rcClient.right - rcClient.left) + x;
              *pOffscreenBits = 0;
            }
          }
        }
        ...
        if (m_bLayered) {
          for (int i = 0; i < m_aNativeWindow.GetSize();) {
            ...
            ::SendMessage(hChildWnd, WM_PRINT, (WPARAM)hChildMemDC,
                          (LPARAM)(PRF_CHECKVISIBLE | PRF_CHILDREN |
          ...
          if (m_diLayered.pImageInfo != NULL) {
            if (m_hbmpBackground == NULL) {
              ...
              m_hbmpBackground = CRenderEngine::CreateARGB32Bitmap(
                  m_hDcPaint, dwWidth, dwHeight, &m_pBackgroundBits);
              ...
              CRenderEngine::DrawImage(m_hDcBackground, this, rcLayeredClient,
                                       rcLayeredClient, m_diLayered);
    ...
    case WM_TIMER: {
      ...
      for (int i = 0; i < m_aTimers.GetSize(); i++) {
        ...
        if (pTimer->hWnd == m_hWndPaint &&
            pTimer->uWinTimer == LOWORD(wParam) && pTimer->bKilled == false) {
          TEventUI event = {0};
    ...
    case WM_MOUSEHOVER: {
      ...
      //修改在CListElementUI 有提示
      //子项无提示下无法跟随移动！（按理说不应该移动的）
    ...
    case WM_MOUSELEAVE: {
      ...
      if (m_bMouseTracking) {
        ...
        if (!::IsIconic(m_hWndPaint) && ::GetActiveWindow() == m_hWndPaint &&
            ::PtInRect(&rcWnd, pt)) {
          if (::SendMessage(m_hWndPaint, WM_NCHITTEST, 0,
                            MAKELPARAM(pt.x, pt.y)) == HTCLIENT) {
            ::ScreenToClient(m_hWndPaint, &pt);
          ...
          if (m_hWndPaint == hParentWnd) {
            for (int i = 0; i < m_aNativeWindow.GetSize(); i++) {
              if (static_cast<HWND>(m_aNativeWindow[i]) == hWnd) {
                if (static_cast<CControlUI*>(m_aNativeWindowControl[i]) !=
                    m_pFocus) {
                  SetFocus(static_cast<CControlUI*>(m_aNativeWindowControl[i]),
                           false);
                }
                break;
              }
    ...
    case WM_NOTIFY: {
      ...
      if (lpNMHDR != NULL)
        lRes =
            ::SendMessage(lpNMHDR->hwndFrom, OCM__BASE + uMsg, wParam, lParam);
      return true;
    } break;
    case WM_COMMAND: {
      if (lParam == 0)
...
bool CPaintManagerUI::InitControls(CControlUI* pControl,
...
bool CPaintManagerUI::RenameControl(CControlUI* pControl, LPCTSTR pstrName) {
  ASSERT(pControl);
  if (pControl == NULL || pControl->GetManager() != this || pstrName == NULL ||
      *pstrName == _T('\0'))
    return false;
  if (pControl->GetName() == pstrName)
    return true;
  if (NULL != FindControl(pstrName))
    return false;
...
bool CPaintManagerUI::AddOptionGroup(LPCTSTR pStrGroupName,
                                     CControlUI* pControl) {
  if (pControl == NULL || pStrGroupName == NULL)
    return false;
...
CDuiPtrArray* CPaintManagerUI::GetOptionGroup(LPCTSTR pStrGroupName) {
  LPVOID lp = m_mOptionGroup.Find(pStrGroupName);
  if (lp)
    return static_cast<CDuiPtrArray*>(lp);
  return NULL;
}
...
void CPaintManagerUI::SetFocus(CControlUI* pControl, bool bFocusWnd) {
  ...
  if (bFocusWnd && hFocusWnd != m_hWndPaint && pControl != m_pFocus &&
      !m_bNoActivate)
    ::SetFocus(m_hWndPaint);
  // Already has focus?
  if (pControl == m_pFocus)
...
bool CPaintManagerUI::SetTimer(CControlUI* pControl,
                               UINT nTimerID,
                               UINT uElapse) {
...
void CPaintManagerUI::PostAsyncNotify() {
  if (!m_bAsyncNotifyPosted) {
    ...
    ::PostMessage(m_hWndPaint, WM_DELAYED_CLEANUP, 0, 0L);
      ...
      if (data != NULL) {
        if (!data->sResType.IsEmpty()) {
          if (isdigit(*bitmap)) {
            ...
            pNewData = CRenderEngine::LoadImage(
                bitmap, data->sResType.GetData(), data->dwMask);
          }
        } else {
  ...
  if (bShared || m_bForceUseSharedRes) {
    ...
    if (pDefaultAttr != NULL) {
      CDuiString* pOldDefaultAttr =
          static_cast<CDuiString*>(m_SharedResInfo.m_AttrHash.Set(
              pStrControlName, (LPVOID)pDefaultAttr));
      if (pOldDefaultAttr)
...
LPCTSTR CPaintManagerUI::GetDefaultAttributeList(
    LPCTSTR pStrControlName) const {
  CDuiString* pDefaultAttr =
      static_cast<CDuiString*>(m_ResInfo.m_AttrHash.Find(pStrControlName));
  if (!pDefaultAttr)
    pDefaultAttr = static_cast<CDuiString*>(
        m_SharedResInfo.m_AttrHash.Find(pStrControlName));
  if (pDefaultAttr)
...
bool CPaintManagerUI::RemoveDefaultAttributeList(LPCTSTR pStrControlName,
                                                 bool bShared) {
  if (bShared) {
    CDuiString* pDefaultAttr = static_cast<CDuiString*>(
        m_SharedResInfo.m_AttrHash.Find(pStrControlName));
    if (!pDefaultAttr)
      return false;
...
void CPaintManagerUI::AddWindowCustomAttribute(LPCTSTR pstrName,
                                               LPCTSTR pstrAttr) {
...
LPCTSTR CPaintManagerUI::GetWindowCustomAttribute(LPCTSTR pstrName) const {
  ...
  CDuiString* pCostomAttr =
      static_cast<CDuiString*>(m_mWindowAttrHash.Find(pstrName));
  if (pCostomAttr)
...
CDuiString CPaintManagerUI::GetWindowXML() {
  ...
  //    if(LPCTSTR key = m_SharedResInfo.m_CustomFonts.GetAt(i)) {
  //        pFontInfo =
  //        static_cast<CDuiString*>(m_SharedResInfo.m_CustomFonts.Find(key))->GetData();
  ...
  //    if(LPCTSTR key = m_ResInfo.m_CustomFonts.GetAt(i)) {
  //        sDefaultAttr =
  //        static_cast<CDuiString*>(m_ResInfo.m_CustomFonts.Find(key))->GetData();
  ...
  for (int i = 0; i < m_ResInfo.m_AttrHash.GetSize(); i++) {
    if (LPCTSTR key = m_ResInfo.m_AttrHash.GetAt(i)) {
      sDefaultAttr =
          static_cast<CDuiString*>(m_ResInfo.m_AttrHash.Find(key))->GetData();
      sDefaultAttr.Replace(_T("\""), _T("&quot;"));
      sWindowXML.Append(_T("\n\t<Default name=\" "));
      sWindowXML.Append(key);
      sWindowXML.Append(_T("\" value=\" "));
...
void CPaintManagerUI::AddMultiLanguageString(int id,
                                             LPCTSTR pStrMultiLanguage) {
  TCHAR idBuffer[16];
  ::ZeroMemory(idBuffer, sizeof(idBuffer));
  _itot(id, idBuffer, 10);
  ...
  if (pMultiLanguage != NULL) {
    CDuiString* pOldMultiLanguage =
        static_cast<CDuiString*>(m_SharedResInfo.m_MultiLanguageHash.Set(
            idBuffer, (LPVOID)pMultiLanguage));
...
LPCTSTR CPaintManagerUI::GetMultiLanguageString(int id) {
  ...
  CDuiString* pMultiLanguage = static_cast<CDuiString*>(
      m_SharedResInfo.m_MultiLanguageHash.Find(idBuffer));
  if (pMultiLanguage)
...
void CPaintManagerUI::RemoveAllMultiLanguageString() {
  ...
  for (int i = 0; i < m_SharedResInfo.m_MultiLanguageHash.GetSize(); i++) {
    if (LPCTSTR key = m_SharedResInfo.m_MultiLanguageHash.GetAt(i)) {
      pMultiLanguage = static_cast<CDuiString*>(
          m_SharedResInfo.m_MultiLanguageHash.Find(key));
      if (pMultiLanguage)
        delete pMultiLanguage;
    }
  }
...
void CPaintManagerUI::ProcessMultiLanguageTokens(
  ...
  while (iPos >= 0) {
    if (pStrMultiLanguage.GetAt(iPos + 1) == _T('{')) {
      int iEndPos = iPos + 2;
      while (isdigit(pStrMultiLanguage.GetAt(iEndPos)))
        iEndPos++;
      if (pStrMultiLanguage.GetAt(iEndPos) == '}') {
        LPCTSTR pStrTemp = CPaintManagerUI::GetMultiLanguageString(
            (UINT)_ttoi(pStrMultiLanguage.GetData() + iPos + 2));
        if (pStrTemp)
          pStrMultiLanguage.Replace(
              pStrMultiLanguage.Mid(iPos, iEndPos - iPos + 1), pStrTemp);
      }
    }
...
CControlUI* CPaintManagerUI::FindSubControlByPoint(CControlUI* pParent,
                                                   POINT pt) const {
  if (pParent == NULL)
    pParent = GetRoot();
  ASSERT(pParent);
...
CControlUI* CPaintManagerUI::FindSubControlByName(CControlUI* pParent,
  ...
  if (pParent == NULL)
    pParent = GetRoot();
  ASSERT(pParent);
  return pParent->FindControl(__FindControlFromName, (LPVOID)pstrName,
                              UIFIND_ALL);
}
...
CControlUI* CPaintManagerUI::FindSubControlByClass(CControlUI* pParent,
  ...
  return pParent->FindControl(__FindControlFromClass, (LPVOID)pstrClass,
                              UIFIND_ALL);
}

CDuiPtrArray* CPaintManagerUI::FindSubControlsByClass(CControlUI* pParent,
                                                      LPCTSTR pstrClass) {
  if (pParent == NULL)
    pParent = GetRoot();
  ASSERT(pParent);
...
CControlUI* CALLBACK
CPaintManagerUI::__FindControlFromNameHash(CControlUI* pThis, LPVOID pData) {
...
CControlUI* CALLBACK
CPaintManagerUI::__FindControlFromCount(CControlUI* /*pThis*/, LPVOID pData) {
...
CControlUI* CALLBACK
CPaintManagerUI::__FindControlFromShortcut(CControlUI* pThis, LPVOID pData) {
  if (!pThis->IsVisible())
    return NULL;
...
CControlUI* CALLBACK
CPaintManagerUI::__FindControlsFromUpdate(CControlUI* pThis, LPVOID pData) {
...
bool CPaintManagerUI::TranslateAccelerator(LPMSG pMsg) {
  for (int i = 0; i < m_aTranslateAccelerator.GetSize(); i++) {
    LRESULT lResult =
        static_cast<ITranslateAccelerator*>(m_aTranslateAccelerator[i])
      ...
      while (hTempParent) {
        if (pMsg->hwnd == pT->GetPaintWindow() ||
          ...
          pT->PreMessageHandler(pMsg->message, pMsg->wParam, pMsg->lParam,
                                lRes);
          // 					if( pT->PreMessageHandler(pMsg->message,
