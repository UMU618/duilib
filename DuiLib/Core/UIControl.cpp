...
CControlUI::CControlUI()
  ...
  m_piFloatPercent.left = m_piFloatPercent.top = m_piFloatPercent.right =
      m_piFloatPercent.bottom = 0.0f;
}
...
bool CControlUI::DrawImage(HDC hDC, TDrawInfo& drawInfo) {
  return CRenderEngine::DrawImage(hDC, m_pManager, m_rcItem, m_rcPaint,
                                  drawInfo);
}
...
void CControlUI::SetPos(RECT rc, bool bNeedInvalidate) {
  ...
  if (m_bFloat) {
    ...
    if (pParent != NULL) {
      ...
      RECT rcPercent = {(LONG)(width * m_piFloatPercent.left),
...
CControlUI* CControlUI::FindControl(FINDCONTROLPROC Proc,
                                    LPVOID pData,
                                    UINT uFlags) {
  ...
  if (pResult == NULL && m_pCover != NULL) {
    /*if( (uFlags & UIFIND_HITTEST) == 0 || true)*/ pResult =
        m_pCover->FindControl(Proc, pData, uFlags);
  }
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue) {
  if (_tcscmp(pstrName, _T("pos")) == 0) {
    ...
    SetPadding(rcPadding);
             ...
             _tcscmp(pstrName, _T("bkcolor1")) == 0) {
...
bool CControlUI::DoPaint(HDC hDC,
                         const RECT& rcPaint,
  ...
  if (m_dwBackColor != 0) {
    if (m_dwBackColor2 != 0) {
      if (m_dwBackColor3 != 0) {
        ...
        CRenderEngine::DrawGradient(hDC, rc, GetAdjustColor(m_dwBackColor),
                                    GetAdjustColor(m_dwBackColor2), true, 8);
        rc.top = rc.bottom;
        rc.bottom = m_rcItem.bottom;
        CRenderEngine::DrawGradient(hDC, rc, GetAdjustColor(m_dwBackColor2),
...
void CControlUI::PaintBorder(HDC hDC,
  ...
  if (border_sizes.left > 0 &&
      (m_cxyBorderRound.cx > 0 || m_cxyBorderRound.cy > 0)) {
    // 画圆角边框
    ...
    if (border_sizes.left > 0) {
      ...
      if (1 == border_sizes.left) {
        ...
        CRenderEngine::DrawLine(hDC, border, border_sizes.left,
                                GetAdjustColor(border_color), border_style);
      } else {
        border.right = border.left + border_sizes.left;
        CRenderEngine::DrawColor(hDC, border, GetAdjustColor(border_color));
      }
    }
    if (border_sizes.top > 0) {
      ...
      if (1 == border_sizes.top) {
        ...
        CRenderEngine::DrawLine(hDC, border, border_sizes.top,
                                GetAdjustColor(border_color), border_style);
      } else {
        border.bottom = border.top + border_sizes.top;
        CRenderEngine::DrawColor(hDC, border, GetAdjustColor(border_color));
      }
    }
    if (border_sizes.right > 0) {
      ...
      if (1 == border_sizes.right) {
        ...
        CRenderEngine::DrawLine(hDC, border, border_sizes.right,
                                GetAdjustColor(border_color), border_style);
      } else {
        border.left = border.right - border_sizes.right;
        CRenderEngine::DrawColor(hDC, border, GetAdjustColor(border_color));
      }
    }
    if (border_sizes.bottom > 0) {
      ...
      if (1 == border_sizes.bottom) {
        ...
        CRenderEngine::DrawLine(hDC, border, border_sizes.bottom,
                                GetAdjustColor(border_color), border_style);
      } else {
        border.top = border.bottom - border_sizes.bottom;
        CRenderEngine::DrawColor(hDC, border, GetAdjustColor(border_color));
      }
    }
  }
...
void CControlUI::PaintBorder(HDC hDC) {
  ...
  // if(m_rcBorderSize.left > 0 && (m_dwBorderColor != 0 || m_dwFocusBorderColor
  // != 0)) { 	if( m_cxyBorderRound.cx > 0 || m_cxyBorderRound.cy > 0
  //)//画圆角边框
  //	{
  //		if (IsFocused() && m_dwFocusBorderColor != 0)
  //			CRenderEngine::DrawRoundRect(hDC, m_rcItem, m_rcBorderSize.left,
  //m_cxyBorderRound.cx, m_cxyBorderRound.cy,
  //GetAdjustColor(m_dwFocusBorderColor), m_nBorderStyle); 		else
  //			CRenderEngine::DrawRoundRect(hDC, m_rcItem, m_rcBorderSize.left,
  //m_cxyBorderRound.cx, m_cxyBorderRound.cy, GetAdjustColor(m_dwBorderColor),
  //m_nBorderStyle);
  //	}
  //	else {
  //		if (m_rcBorderSize.right == m_rcBorderSize.left &&
  //m_rcBorderSize.top == m_rcBorderSize.left && m_rcBorderSize.bottom ==
  //m_rcBorderSize.left) { 			if (IsFocused() &&
  //m_dwFocusBorderColor != 0)
