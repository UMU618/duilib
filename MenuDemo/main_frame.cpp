...
// UMU
const TCHAR* const kWindowInit = {DUI_MSGTYPE_WINDOWINIT};
const TCHAR* const kClick = {DUI_MSGTYPE_CLICK};
const TCHAR* const kTimer = {DUI_MSGTYPE_TIMER};
...
LRESULT MainFrame::OnSysCommand(UINT uMsg,
                                WPARAM wParam,
                                LPARAM lParam,
                                BOOL& bHandled) {
...
void MainFrame::Notify(TNotifyUI& msg) {
  if (_tcsicmp(msg.sType, kWindowInit) == 0) {
    ...
    if (_tcsicmp(msg.pSender->GetName(), kCloseButtonControlName) == 0) {
...
#endif
    } else if (_tcsicmp(msg.pSender->GetName(), kRestoreButtonControlName) ==
               0) {
#if defined(UNDER_CE)
      ...
      // UMU: 如果不设置 deleter，则当 DuiLib.DLL 和 EXE 不使用相同 CRT DLL 时，内存将在 EXE 分配，而在 DLL 释放，然后崩溃！
      CMenuWnd* pMenu = new CMenuWnd(m_hWnd, [](void* mem) { delete mem; });
...
LRESULT MainFrame::HandleCustomMessage(UINT uMsg,
                                       WPARAM wParam,
                                       LPARAM lParam,
                                       BOOL& bHandled) {
