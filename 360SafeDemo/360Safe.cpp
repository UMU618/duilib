...
class C360SafeFrameWnd : public CWindowWnd, public INotifyUI {
 public:
  ...
  LRESULT OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
    ...
    // HTTOPLEFT; 				if( pt.x > rcClient.right -
    // rcSizeBox.right ) return HTTOPRIGHT; 				return
    // HTTOP;
    // 			}
    // 			else if( pt.y > rcClient.bottom - rcSizeBox.bottom ) {
    // 				if( pt.x < rcClient.left + rcSizeBox.left ) return
    // HTBOTTOMLEFT;
    // 				if( pt.x > rcClient.right - rcSizeBox.right ) return
    // HTBOTTOMRIGHT; 				return HTBOTTOM;
    // 			}
    // 			if( pt.x < rcClient.left + rcSizeBox.left ) return
    // HTLEFT;
    // 			if( pt.x > rcClient.right - rcSizeBox.right ) return
...
int APIENTRY WinMain(HINSTANCE hInstance,
  ...
  if (pFrame == NULL)
    return 0;
  pFrame->Create(NULL, _T("360安全卫士"), UI_WNDSTYLE_FRAME, 0L, 0, 0, 800,
                 572);
  pFrame->CenterWindow();
