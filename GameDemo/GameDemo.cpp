...
class CLoginFrameWnd : public CWindowWnd,
                       public INotifyUI,
 ...
 public:
  ...
  void Init() {
    CComboUI* pAccountCombo =
        static_cast<CComboUI*>(m_pm.FindControl(_T("accountcombo")));
  ...
  LRESULT OnNcActivate(UINT uMsg,
                       WPARAM wParam,
                       LPARAM lParam,
                       BOOL& bHandled) {
    if (::IsIconic(*this))
  ...
  LRESULT OnNcCalcSize(UINT uMsg,
                       WPARAM wParam,
                       LPARAM lParam,
                       BOOL& bHandled) {
    return 0;
  ...
  LRESULT OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
    ...
    if (!::IsIconic(*this) &&
      ...
      rcWnd.bottom++;
      HRGN hRgn =
          ::CreateRoundRectRgn(rcWnd.left, rcWnd.top, rcWnd.right, rcWnd.bottom,
...
class CGameFrameWnd : public CWindowWnd,
                      public INotifyUI,
 ...
 public:
  ...
  void Init() {
    ...
    CActiveXUI* pActiveXUI =
        static_cast<CActiveXUI*>(m_pm.FindControl(_T("ie")));
    if (pActiveXUI) {
      IWebBrowser2* pWebBrowser = NULL;
      pActiveXUI->GetControl(IID_IWebBrowser2, (void**)&pWebBrowser);
      if (pWebBrowser != NULL) {
        pWebBrowser->Navigate(L"https://github.com/UMU618/duilib", NULL, NULL,
                              NULL, NULL);
  ...
  void OnPrepare() {
    GameListUI* pGameList =
        static_cast<GameListUI*>(m_pm.FindControl(_T("gamelist")));
    ...
    pCategoryNode =
        pGameList->AddNode(_T("{x 4}{i gameicons.png 18 3}{x 4}推荐游戏"));
    for (int i = 0; i < 4; ++i) {
      pGameNode = pGameList->AddNode(
          _T("{x 4}{i gameicons.png 18 10}{x 4}四人斗地主"), pCategoryNode);
      for (int i = 0; i < 3; ++i) {
        pServerNode = pGameList->AddNode(
            _T("{x 4}{i gameicons.png 18 10}{x 4}测试服务器"), pGameNode);
        for (int i = 0; i < 3; ++i) {
          pRoomNode = pGameList->AddNode(
              _T("{x 4}{i gameicons.png 18 10}{x 4}测试房间"), pServerNode);
        }
      }
    }
    pCategoryNode = pGameList->AddNode(
        _T("{x 4}{i gameicons.png 18 3}{x 4}最近玩过的游戏"));
    for (int i = 0; i < 2; ++i) {
      pGameList->AddNode(_T("三缺一"), pCategoryNode);
    }
    pCategoryNode =
        pGameList->AddNode(_T("{x 4}{i gameicons.png 18 3}{x 4}棋牌游戏"));
    for (int i = 0; i < 8; ++i) {
      pGameList->AddNode(_T("双扣"), pCategoryNode);
    }
    pCategoryNode =
        pGameList->AddNode(_T("{x 4}{i gameicons.png 18 3}{x 4}休闲游戏"));
    for (int i = 0; i < 8; ++i) {
      pGameList->AddNode(_T("飞行棋"), pCategoryNode);
    }

    CListUI* pUserList =
        static_cast<CListUI*>(m_pm.FindControl(_T("userlist")));
    ...
    pLoginFrame->Create(m_hWnd, _T(""), UI_WNDSTYLE_DIALOG, 0, 0, 0, 0, 0,
                        NULL);
  ...
  void SendChatMessage() {
    CEditUI* pChatEdit =
        static_cast<CEditUI*>(m_pm.FindControl(_T("chatEdit")));
    if (pChatEdit == NULL)
      return;
    pChatEdit->SetFocus();
    if (pChatEdit->GetText().IsEmpty())
      return;

    CRichEditUI* pRichEdit =
        static_cast<CRichEditUI*>(m_pm.FindControl(_T("chatmsglist")));
    if (pRichEdit == NULL)
      return;
    ...
    if (msg.sType == _T("windowinit"))
      ...
      if (name == _T("quitbtn")) {
        ...
        if (pFontInfo->iSize < 18) {
          ...
          if (pFontInfo)
            m_pm.SetDefaultFont(pFontInfo->sFontName, pFontInfo->iSize,
                                pFontInfo->bBold, pFontInfo->bUnderline,
                                pFontInfo->bItalic);
        } else {
        ...
        if (pControl) {
          pControl->Activate();
          pControl =
              static_cast<COptionUI*>(m_pm.FindControl(_T("roomswitch")));
          if (pControl)
            pControl->SetVisible(false);
        }
      ...
      if (name == _T("hallswitch")) {
        CTabLayoutUI* pControl =
            static_cast<CTabLayoutUI*>(m_pm.FindControl(_T("switch")));
        ...
        if (pControl && pControl->GetCurSel() != 1) {
          ...
          DeskListUI* pDeskList =
              static_cast<DeskListUI*>(m_pm.FindControl(_T("destlist")));
          pDeskList->SetFocus();
          CRichEditUI* pRichEdit =
              static_cast<CRichEditUI*>(m_pm.FindControl(_T("chatmsglist")));
          if (pRichEdit) {
  ...
  LRESULT OnSysCommand(UINT uMsg,
    ...
    if (::IsZoomed(*this) != bZoomed) {
      if (!bZoomed) {
        CControlUI* pControl =
            static_cast<CControlUI*>(m_pm.FindControl(_T("maxbtn")));
        if (pControl)
          pControl->SetVisible(false);
