  ...
  void Notify(TNotifyUI& msg) {
    if (msg.sType == _T("click")) {
      ...
      if (pFlash != NULL) {
        ...
        pFlash->put_Movie(_bstr_t(umu::apppath::GetProductDirectory() +
                                  _T("_skin\\FlashRes\\test.swf")));
        pFlash->DisableLocalSecurity();
        pFlash->put_AllowScriptAccess(L"always");
        BSTR response;
        pFlash->CallFunction(
            L"<invoke name=\"setButtonText\" "
            L"returntype=\"xml\"><arguments><string>Click "
            L"me!</string></arguments></invoke>",
            &response);
        pFlash->Release();
      }
    }
