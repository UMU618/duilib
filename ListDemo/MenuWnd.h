...
inline HBITMAP CreateMyBitmap(HDC hDC, int cx, int cy, COLORREF** pBits) {
  ...
  HBITMAP hBitmap =
      CreateDIBSection(hDC, lpbiSrc, DIB_RGB_COLORS, (void**)pBits, NULL, NULL);
  delete[] lpbiSrc;
  return hBitmap;
}
 ...
 public:
  ...
  void RePaint() {
    ...
    HBITMAP hbmpBackground =
        CreateMyBitmap(hDcPaint, width, height, &pBackgroundBits);
    ::ZeroMemory(pBackgroundBits, width * height * 4);
    ...
    POINT ptSrc = {0, 0};
    UpdateLayeredWindow(m_hWnd, hDcPaint, &ptPos, &sizeWnd, hDcBackground,
...
class CMyMenuWnd : public CWindowWnd, public INotifyUI {
 public:
  ...
  void Notify(TNotifyUI& msg) {
    if (msg.sType == _T("itemselect")) {
      ...
      if (msg.pSender->GetName() == _T("menu_Delete")) {
        if (m_pOwner)
          m_pOwner->GetManager()->SendNotify(m_pOwner, _T("menu_Delete"), 0, 0,
                                             true);
      }
    }
  }
  ...
  HWND Create(HWND hwndParent,
    ...
    return CWindowWnd::Create(hwndParent, pstrName, dwStyle, dwExStyle, x, y,
                              cx, cy, hMenu);
  }
  ...
  LRESULT OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
    if (!::IsIconic(*this)) {
      ...
      HRGN hRgn = ::CreateRectRgn(rcWnd.left + 8, rcWnd.top + 8,
                                  rcWnd.right - 8, rcWnd.bottom - 8);
