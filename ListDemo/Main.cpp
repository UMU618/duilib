...
class ListMainForm : public CWindowWnd,
                     public INotifyUI,
 ...
 public:
  ...
  void OnSearch() {
    ...
    HANDLE hThread = CreateThread(NULL, 0, &ListMainForm::Search, (LPVOID)prama,
                                  0, &dwThreadID);
  }
  /*
   * 关键的回调函数，IListCallbackUI
   * 中的一个虚函数，渲染时候会调用,在[1]中设置了回调对象
   */
  LPCTSTR GetItemText(CControlUI* pControl, int iIndex, int iSubItem) {
    ...
    switch (iSubItem) {
      ...
      case 1: {
        ...
        ::MultiByteToWideChar(CP_ACP, 0, domain[iIndex].c_str(), -1,
                              (LPWSTR)lpText, iLen);
      ...
      case 2: {
        ...
        LPWSTR lpText = new WCHAR[iLen + 1];
        ::ZeroMemory(lpText, (iLen + 1) * sizeof(WCHAR));
        ::MultiByteToWideChar(CP_ACP, 0, desc[iIndex].c_str(), -1,
                              (LPWSTR)lpText, iLen);
        _stprintf_s(szBuf, lpText);
        delete[] lpText;
#else
        _stprintf_s(szBuf, desc[iIndex].c_str());
#endif
      } break;
    }
  ...
  LRESULT OnAddListItem(UINT uMsg,
                        WPARAM wParam,
                        LPARAM lParam,
                        BOOL& bHandled) {
  ...
  LRESULT OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
    ...
    if (!::IsIconic(*this) &&
      ...
      rcWnd.right++;
      rcWnd.bottom++;
      RECT rc = {rcWnd.left, rcWnd.top + szRoundCorner.cx, rcWnd.right,
                 rcWnd.bottom};
      HRGN hRgn1 = ::CreateRectRgnIndirect(&rc);
      HRGN hRgn2 = ::CreateRoundRectRgn(rcWnd.left, rcWnd.top, rcWnd.right,
...
int APIENTRY _tWinMain(HINSTANCE hInstance,
  ...
  CPaintManagerUI::SetResourcePath(umu::apppath::GetProductDirectory() + _T("_skin"));
  CPaintManagerUI::SetResourceZip(_T("ListRes.zip"));

  ListMainForm* pFrame = new ListMainForm();
  if (pFrame == NULL)
    return 0;
  pFrame->Create(NULL, _T("ListDemo"), UI_WNDSTYLE_FRAME,
                 WS_EX_STATICEDGE | WS_EX_APPWINDOW, 0, 0, 600, 320);
  pFrame->CenterWindow();
